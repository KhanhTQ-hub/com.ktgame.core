...
namespace com.ktgame.core.di
{
	internal static class ActivatorFactoryManager
	{
		...
		private static IActivatorFactory GetFactory()
		{
			...
			{
				case ScriptingBackend.Backend.Mono:
					return new MonoActivatorFactory();
				case ScriptingBackend.Backend.IL2CPP:
					return new IL2CPPActivatorFactory();
				case ScriptingBackend.Backend.Undefined:
					throw new Exception("UndefinedRuntimeScriptingBackend");
				default:
					throw new Exception($"UnhandledRuntimeScriptingBackend {ScriptingBackend.Current}");
			}
		}
	}
}
